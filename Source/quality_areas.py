...
class QualityAreas(object):
    """
        Create a rectangular grid of quality areas which cover the entire frame. For each patch the image quality of all
        frames is ranked, and a certain percentage of the best frames is stacked to produce the final image. Since
        the seeing deteriorates the image quality in a non-uniform way across the image, the list of best frames is
        different for different quality areas.
    ...
    def __init__(self, configuration, frames, align_frames, alignment_points):
        ...
        # Because in the general case the frame cannot be divided evenly, extend the last patch to extend up do the
        # frame border.
        ...
        for index_y, y_low in enumerate(self.y_lows):
            ...
            for index_x, x_low in enumerate(self.x_lows):
                # For each quality area, store coordinate bounds and empty lists for alignment points and frame
                # qualities in a dictionary. The dictionaries are stored in a a 2D list structure (row-wise).
        ...
        # Cycle through all alignment points. For each point append its index to the alignment point list of the
        # quality area which contains the point.
    ...
    def select_best_frames(self):
        """
        For each quality area, rank all frames by their local image quality. For quality areas containing at least one
        alignment point, use their "local contrast" as the ranking criterion. For areas which do not contain any
        alignment point, copy the frame ranks from the nearest quality are with alignment points. The rationale for
        this differentiation is that areas without alignment points are either empty space or very smooth surface
        sections where the contrast measurement does not make much sense.
        ...
        for frame in self.frames.frames_mono:
            ...
            for index_y, quality_area_row in enumerate(self.quality_areas):
                for index_x, quality_area in enumerate(quality_area_row):
                    # If the alignment point list of the quality area is non-empty, compute the local contrast.
        ...
        # For quality areas with alignment points, sort the computed quality ranks in descending order.
        for index_y, quality_area_row in enumerate(self.quality_areas):
            for index_x, quality_area in enumerate(quality_area_row):
                if quality_area['alignment_point_indices']:
        ...
        # For quality areas without alignment points, use method "best_frame_indices_in_empty_areas" to copy
        # ranks from the nearest quality are with alignment points.
        for index_y, quality_area_row in enumerate(self.quality_areas):
            for index_x, quality_area in enumerate(quality_area_row):
    ...
    def best_frame_indices_in_empty_areas(self, index_y, index_x):
        """
        For a quality area without any alignment point, find the closest quality area with alignment points and return
        its list of frame indices ranked by the local frame quality in decreasing order.
        ...
        :return: frame index list, ranked by the image quality at the closest quality area with alignment points
        """
        ...
        for distance in arange(1, max(self.y_dim, self.x_dim)):
            ...
            for (compare_x, compare_y) in circle:
                # If the coordinates are within the quality area grid, and if the area at this location has a non-empty
                # list of alignment points, return its list.
        ...
        # This should never happen, because it means that there is not any quality area with an alignment point.
        return []

    def truncate_best_frames(self):
        """
        Compute the number of frames to be stacked and for all quality areas truncate the ranking lists with the best
        frame indices.
        ...
        # This is a precaution measure: Do not take more frames than elements in the shortest ranking list.
        self.stack_size = min(max_frames, min(
            [min([len(self.quality_areas[j][i]['best_frame_indices']) for i in arange(self.x_dim)]) for j in
             arange(self.y_dim)]))

        # For all quality areas: Truncate the "best frame indices" list to the uniform selected stack size.
        for index_y, quality_area_row in enumerate(self.quality_areas):
            for index_x, quality_area in enumerate(quality_area_row):
