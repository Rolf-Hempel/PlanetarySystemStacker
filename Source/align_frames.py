...
class AlignFrames(object):
    """
        Based on a list of frames and a (parallel) list of frame quality values, an averaged reference frame is created
        and all frames are aligned with this frame. The alignment is performed on a small rectangular area where
        structure is optimal in both x and y directions.
    ...
    def select_alignment_rect(self, scale_factor):
        """
        Using the frame with the highest rank (sharpest image), select the rectangular patch where structure is best in
        both x and y directions. The size of the patch is the size of the frame divided by "scale_factor" in both
        coordinate directions.

        :param scale_factor: Ratio of the size of the frame and the alignment patch in both coordinate directions
        :return: A four tuple (x_low, x_high, y_low, y_high) with pixel coordinates of the optimal patch
        """
        ...
        # Compute for all locations in the frame the "quality measure" and find the place with the maximum value.
    ...
    def align_frames(self):
        """
        Compute the displacement of all frames relative to the sharpest frame using the alignment rectangle.
        ...
        # From the sharpest frame cut out the alignment rectangle. The shifts of all other frames will be computed
        # relativ to this patch.
        ...
        for index, frame in enumerate(self.frames_mono):
            ...
            # For all other frames: Cut out the alignment patch and compute its translation relative to the reference.
            else:
        ...
        # Compute the shape of the area contained in all frames in the form [[y_low, y_high], [x_low, x_high]]
    ...
    def average_frame(self, frames, shifts):
        """
        Compute an averaged frame from a list of (monochrome) frames along with their corresponding shift values.
        ...
        # Create an empty numpy buffer. The first dimension is the frame index, the second and third dimenstions are
